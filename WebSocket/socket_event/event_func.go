...
func (s *socketEvent) FetchData(sck socketio.Conn, msg string) {
	...
	socket_resp.DoEmit(sck, constants.FetchEvent, "fetch talents on progress")
...
func (s *socketEvent) VerifyData(sck socketio.Conn, msg string) {
	fmt.Printf("%+v\n", msg)
	rlog.Info(sck.ID())
	...
	socket_resp.DoEmit(sck, constants.VerifyEvent, "verify talents on progress")

	if err := services.VerifService.VerifTalentService(req.StoryLimit, req.Url, &sck); err != nil {
		socket_resp.DoEmitError(sck, err)
		return
	}
}
...
func (s *socketEvent) Test(sck socketio.Conn, msg string) {
	...
	socket_resp.DoEmit(sck, constants.TestEvent, "pong")
}
